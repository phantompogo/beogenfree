...
export const useVideoGenerator = () => {
  ...
  const [selectedModel, setSelectedModel] = useState<string>(VEO_MODELS[0].id);
  const [aspectRatio, setAspectRatio] = useState<string>(VEO_MODELS[0].options.aspectRatios[0]);
  const [duration, setDuration] = useState<number>(VEO_MODELS[0].options.durations[0]);
  const [videoCount, setVideoCount] = useState<number>(VEO_MODELS[0].options.videoCounts[0]);
  ...
  // Effect to reset options when model changes
  useEffect(() => {
      const modelDetails = VEO_MODELS.find(m => m.id === selectedModel);
      if (modelDetails) {
          // Reset options to default for the selected model
          setAspectRatio(modelDetails.options.aspectRatios[0]);
          setDuration(modelDetails.options.durations[0]);
          setVideoCount(modelDetails.options.videoCounts[0]);
          
          // Clear image if the model does not support it
          if (!modelDetails.supportsImage) {
              setImageFile(null);
          }
      }
  }, [selectedModel]);
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (imageFile && modelDetails?.supportsImage) {
        imageBase64 = await fileToBase64(imageFile);
        mimeType = imageFile.type;
      }

      const videoBlob = await generateVideo(prompt, selectedModel, {
  ...
  }, [prompt, imageFile, isLoading, selectedModel, aspectRatio, videoCount]);
